...
public class TestClient {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            /*
             * if (args.length % 2 != 0) { throw new MalformedParametersException(); }
             * 
             * for (int i = 0; i < args.length; i++) { if (args[i].equals("-r")) { String
             * reg = args[++i]; String[] val = reg.split(":"); if (val.length > 0) {
             * listReg.add(new InetSocketAddress(val[0], Integer.valueOf(val[1]))); } else {
             * listReg.add(new InetSocketAddress(val[0], DEFAULT_PORT_REG)); } } }
             * 
             * for (int i = 0; i < listReg.size(); i++) { RegistryInterface reg = null; try
             * { reg = (RegistryInterface) Naming .lookup("//" +
             * listReg.get(i).getHostName() + ":" + listReg.get(i).getPort() + "/registry");
             * } catch (RemoteException re) { re.printStackTrace(); reg = null; } catch
             * (NotBoundException nbe) { nbe.printStackTrace(); reg = null; } catch
             * (MalformedURLException mue) { mue.printStackTrace(); reg = null; } if (reg !=
             * null) { registryList.add(reg); } }
             */
            Scanner in = new Scanner(System.in);
            System.out.println("A che registro vuoi inviare la transazione?");
            ...
            case 1:
                ...
                try {
                    reg = (RegistryInterface) Naming.lookup("//192.168.1.72:7867/registry");
            ...
            case 2:
                reg = null;
                try {
                    reg = (RegistryInterface) Naming.lookup("//192.168.1.72:7868/registry");
                } catch (RemoteException re) {
                    re.printStackTrace();
                    reg = null;
                } catch (NotBoundException nbe) {
            ...
            case 3:
                reg = null;
                try {
                    reg = (RegistryInterface) Naming.lookup("//192.168.1.72:7869/registry");
                } catch (RemoteException re) {
                    re.printStackTrace();
                    reg = null;
                } catch (NotBoundException nbe) {
